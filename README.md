...
To assist with the setup process, run `yarn hardhat vars setup`. To set a particular value, such as a BIP-39 mnemonic
...
```sh
yarn run task:deployLock --unlock 100 --value 0.1
```
...
To view the coverage report generated by `yarn run coverage`, just click `Go Live` from the status bar to turn the
server on/off.
